...
import { requireAuth } from "@/modules/auth/require-auth"; // Fixed path matching
...
// 1. GET /api/resources/[resourceId] -> Fetch single resource details (Public)
...
// 2. PUT /api/resources/[resourceId] -> Update resource data nodes (Protected - Author Only)
export const PUT = handleApiRoute(async (req: NextRequest, context: RouteContext) => {
  ...
  if (data.title && data.title.trim() !== resource.title) {
    ...
    const oldSlugParts = resource.slug.split("-");
...
// 3. DELETE /api/resources/[resourceId] -> Safely soft delete records (Protected - Author Only)
